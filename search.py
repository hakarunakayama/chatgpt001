...
def answer_question_nakatsu(question, conversation_history):
    ...
    df = pd.read_csv(os.path.join(cur_dir, 'embeddings/embeddings_nakatsu.csv'))
...
def answer_question_nakayama(question, conversation_history):
    # df = pd.read_csv(os.path.expanduser("/home/ubuntu/prog/chatgpt001/embeddings.csv"))
    # 以下は開発用ローカルディレクトリ（2024/5/17:os.pathモジュールを使ってカレントディレクトリを取得)
    cur_dir = os.path.dirname(os.path.abspath(__file__))
    df = pd.read_csv(os.path.join(cur_dir, 'embeddings/embeddings_nakayama.csv'))
    context = create_context(question, df, max_len=200)
